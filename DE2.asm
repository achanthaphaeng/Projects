...
ORG 0
	...
	JUMP   Sonar_ISR   ; Sonar interrupt (changed)
...
Main:
	...
	SEI    &B0011      ; enable interrupts from source 2 (timer) & source 1 (sonar)
	...
	LOADI  &B00010000
	OUT    SONAREN     ; enable sonar 4
	LOADI  96          ; ~10cm in 1.04mm units
	OUT    SONALARM    ; set alarm distance
	;-----------------------------------------
	;WHAT WE ADDED HERE MOST LIKELY MAY NOT WORK PHASE 4
	;-----------------------------------------
	LOADI  90
	STORE  TFAngle     ; set angle to 45 degrees
	LOADI  &B00100000
	OUT    SONAREN
	IN     DIST5
	OUT    XPOS
...
Turn:                  ; turns robot relative to current angle
	...
	STORE  TFAngle     ; store abosulate angle for future use
TurnLoop:
	IN     Theta
	SUB    TFAngle
